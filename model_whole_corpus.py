...
def get_ngram_statistics(corpus_list, n):
...
def get_word_indices(target_object, n):
    target_object.seek(0)
    corpus_list = []
    #Padding for the edges of the corpus
    corpus_list.append([None for i in range(n)])
...
def get_init_prob(corpus_list, ngram_freqs, conditional_freqs, n):
    target_object.seek(0)
    ...
    for pair in quick_find_pairs:
        ...
        for delta_base in changed_bases:
            if changed_bases[delta_base] == 0:
                continue
            new_base_count = conditional_freqs[delta_base] + changed_bases[delta_base]
            for prev_conditioned in ngram_freqs[delta_base]:
                conditional_count = ngram_freqs[delta_base][prev_conditioned]
                #Undo the old probability contribution
                delta -= conditional_count*log(conditional_count/conditional_freqs[delta_base])
                #And add the new one....
                if new_base_count > 0:
        ...
        for delta_base in changed_conditionals:
            ...
            if new_base_count > 0:
                ...
                for delta_conditioned in cur_cond_dict:
                    ...
                    if count_change == 0:
                        continue
                    old_conditional_count = ngram_freqs[delta_base][delta_conditioned]
...
if __name__ == '__main__':
    ...
    corpus_list, quick_find_words, quick_find_pairs = get_word_indices(target_object, num_before)
    ngram_freqs, conditional_freqs = get_ngram_statistics(corpus_list, num_before)

    #Calculate the initial log probability of the corpus.
    #TODO: Uncomment
    #init_prob = get_init_prob(corpus_list, ngram_freqs, conditional_freqs, num_before)
